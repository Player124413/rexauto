...
def verify_sdk_pin(env):
    ...
    if not rexglue:
        return
    # REXAUTO_SDK_PIN=0 disables the byte-exact check (hand-installed or
    # upstream SDK). Default: enforced -- gui/setup.py downloads the exact build.
    if os.environ.get("REXAUTO_SDK_PIN", "1") == "0":
